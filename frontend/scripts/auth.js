...
async function getUserInfo() {
  ...
  if (cachedEmail) {
    ...
    try {
      ...
      if (payload.exp && payload.exp * 1000 < Date.now()) {
        console.warn('⏰ Token expired!', 'Expired at:', new Date(payload.exp * 1000).toLocaleTimeString(), 'Now:', new Date().toLocaleTimeString());
  ...
  try {
    ...
    if (parts.length !== 3) {
      ...
      const recovered = await refreshAccessToken();
      if (recovered) {
        console.log('✅ Token recovered, retrying getUserInfo...');
        return await getUserInfo();
      }
      return null;
    }
