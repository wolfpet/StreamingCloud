...
const DEFAULT_CONFIG_FILENAME = "site.config.json";
const EXAMPLE_FILENAME = "site.config.example.json";

/**
 * Load and validate site configuration.
 * @param {string} configFilename - Optional config filename (defaults to site.config.json or CDK_SITE_CONFIG env var)
 * @returns {object} The parsed site config
 * @throws {Error} If config file is missing or invalid
 */
function loadSiteConfig(configFilename) {
  const CONFIG_FILENAME = configFilename || process.env.CDK_SITE_CONFIG || DEFAULT_CONFIG_FILENAME;
...
function toStackConfig(siteConfig) {
  return {
    // Cognito
    GOOGLE_OAUTH: siteConfig.cognito?.googleOAuth ?? false,
    ...
    //optional approval for new uploads
    APPROVAL_REQUIRED: siteConfig.administration?.approvalRequired ?? false,
    //optional disabling of public uploads (only allow admins to upload)
    NO_PUBLIC_UPLOADS: siteConfig.administration?.noPublicUploads ?? false,
    // audd.io track recognition
    TRACK_RECOGNITION: siteConfig.audd?.trackRecognition ?? false,
   };
}
