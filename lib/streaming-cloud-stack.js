...
class StreamingCloudStack extends Stack {
  ...
  constructor(scope, id, props) {
    ...
    usersTable.grantReadData(getUserAttributesLambda);
    ...
    // 6a. Deploy brand-override files on top of frontend (if directory exists)
    const brandOverridePath = path.resolve(__dirname, "..", "brand-override");
    if (fs.existsSync(brandOverridePath)) {
      const brandOverrideDeployment = new s3deploy.BucketDeployment(this, "DeployBrandOverride", {
        sources: [s3deploy.Source.asset(brandOverridePath)],
        destinationBucket: websiteBucket,
        distribution: distribution,
        distributionPaths: ["/*"],
        prune: false,
      });
      brandOverrideDeployment.node.addDependency(frontendDeployment);
    }
