  ...
  constructor(scope, id, props) {
    ...
    const userPoolClient = userPool.addClient("UserPoolClient", {
      ...
      supportedIdentityProviders: [
        ...(config.GOOGLE_OAUTH ? [cognito.UserPoolClientIdentityProvider.GOOGLE] : []),
    ...
    // Add Google Identity Provider (optional — set cognito.googleOAuth: true in site config)
    ...
    const addPodcastLambda = new lambda.Function(this, "AddPodcastHandler", {
      ...
      environment: {
        ...
        APPROVAL_REQUIRED: String(config.APPROVAL_REQUIRED),
      },
    });
    ...
    // 3p. Track Recognition Lambda + Step Function (conditional on audd.trackRecognition)
    if (config.TRACK_RECOGNITION) {
      const recognizeTracksLambda = new lambda.Function(this, "RecognizeTracksHandler", {
        runtime: lambda.Runtime.NODEJS_24_X,
        code: lambda.Code.fromAsset("lambda", { assetHashType: cdk.AssetHashType.SOURCE }),
        handler: "recognize_tracks.handler",
        ...
        description: "Splits audio into chunks, recognizes tracks via audd.io, writes tracklist to DynamoDB",
      });

      // Grant DynamoDB write so the lambda can update the tracklist attribute
      podcastTable.grantWriteData(recognizeTracksLambda);

      // 3q. Track Recognition Step Function
      const trackRecognitionStateMachine = new sfn.StateMachine(this, "TrackRecognitionStateMachine", {
        stateMachineName: `${domainPrefix}-TrackRecognition`,
        definitionBody: sfn.DefinitionBody.fromChainable(
          new tasks.LambdaInvoke(this, "RecognizeTracks", {
            lambdaFunction: recognizeTracksLambda,
            payload: sfn.TaskInput.fromObject({
              audioUrl: sfn.JsonPath.stringAt("$.audioUrl"),
              podcastId: sfn.JsonPath.stringAt("$.podcastId"),
              pk: sfn.JsonPath.stringAt("$.pk"),
              timestamp: sfn.JsonPath.stringAt("$.timestamp"),
            }),
          })
      ...
      // Grant the trigger lambda permission to start this state machine
      trackRecognitionStateMachine.grantStartExecution(triggerWaveformLambda);

      // Add the track recognition state machine ARN to the trigger lambda environment
      triggerWaveformLambda.addEnvironment(
        "TRACK_RECOGNITION_STATE_MACHINE_ARN",
        trackRecognitionStateMachine.stateMachineArn
      );
    }
    ...
    // Ensure API Gateway-generated errors (e.g., 401 from authorizer) include CORS headers.
    ...
    // All sources are combined into a single atomic deployment.
    // Brand-override sources (if the directory exists and this is a production
    // deploy using site.config.json) are added AFTER the base frontend source
    // so that they overwrite defaults.
    // env.js is generated last so it always overwrites any local dev copy.
    const brandOverridePath = path.resolve(__dirname, "..", "brand-override");
    const isProductionConfig = !process.env.CDK_SITE_CONFIG || process.env.CDK_SITE_CONFIG === "site.config.json";
    ...
    if (isProductionConfig && fs.existsSync(brandOverridePath)) {
      frontendSources.push(s3deploy.Source.asset(brandOverridePath));
    }
    // env.js is added last so the CDK-generated version always wins
    frontendSources.push(
      s3deploy.Source.data(
        ...
        [
          ...
          `  MIN_FILE_LENGTH_SECONDS: ${siteConfig.upload?.minFileLengthSeconds ?? 0},`,
          `  NO_PUBLIC_UPLOADS: ${siteConfig.administration?.noPublicUploads ?? false}`,
          `};`,
        ].join('\n') + '\n'
      ),
    );
